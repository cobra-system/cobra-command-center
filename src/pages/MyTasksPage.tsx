      ...
      <div className="flex flex-col items-center py-6">
        <div className="relative w-36 h-36">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie data={donutData} innerRadius={50} outerRadius={65} dataKey="value" startAngle={90} endAngle={-270} strokeWidth={0}>
      ...
      <div className="px-4 space-y-4">
        {p0Tasks.length > 0 && (
          <div>
            ...
            <div className="space-y-2">
              {p0Tasks.map(task => (
                <TaskRow key={task.id} task={task} onToggle={() => updateTaskStatus(task.id, "DONE")} onTap={() => navigate(`/my-tasks/${task.id}`)} />
              ))}
            </div>
          </div>
        )}
        ...
        {done.length > 0 && (
          <div>
            ...
            <div className="space-y-2">
              {done.map(task => (
                <TaskRow key={task.id} task={task} completed onToggle={() => updateTaskStatus(task.id, "TODO")} onTap={() => navigate(`/my-tasks/${task.id}`)} />
              ))}
            </div>
          </div>
        )}
        ...
        {myTasks.length === 0 && (
          <div className="text-center py-12"><p className="text-lg text-muted-foreground">אין משימות כרגע 🎉</p></div>
        )}
      </div>
    </div>
  );
...
interface TaskRowProps {
  task: { id: string; title: string; priority: string; notes?: string | null };
  completed?: boolean;
  onToggle: () => void;
  onTap: () => void;
}
  ...
  return (
    <div className="bg-card rounded-xl border p-4 flex items-center gap-3 active:bg-muted/50 transition-colors">
      <button onClick={onToggle} className="shrink-0">
        {completed ? <CheckCircle2 className="h-6 w-6 text-success animate-check-mark" /> : <Circle className="h-6 w-6 text-muted-foreground" />}
      </button>
