      ...
      <div className="bg-card rounded-xl border shadow-sm overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y">
            {filtered.length === 0 ? (
              <tr><td colSpan={10} className="p-8 text-center text-muted-foreground">לא נמצאו מוצרים</td></tr>
            ) : filtered.map(p => (
              <tr key={p.id} className={getRowClass(p.stock_qty, p.monthly_order)}>
                <td className="p-3 font-medium text-foreground">{p.name}</td>
                <td className="p-3 text-muted-foreground font-mono text-xs" dir="ltr">{p.sku}</td>
                <td className="p-3">
                  <span className={`px-2 py-0.5 rounded text-xs font-medium ${
                    p.product_type === "מורכב" ? "bg-accent/15 text-accent" : "bg-muted text-muted-foreground"
                  }`}>{p.product_type}</span>
                </td>
                ...
                <td className="p-3 flex gap-2">
                  <button onClick={() => navigate(`/products/${p.id}`)} className="text-primary hover:text-primary/80"><Eye className="h-4 w-4" /></button>
                  <button className="text-accent hover:text-accent/80"><Edit2 className="h-4 w-4" /></button>
                </td>
              </tr>
            ))}
          </tbody>
