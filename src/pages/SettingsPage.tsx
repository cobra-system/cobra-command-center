...
import { Lock, Users, Mail, CheckCircle, LogOut, Plus, Pencil, Trash2, User, Settings } from "lucide-react";
...
const employeeRoleOptions: { value: Role; label: string }[] = [
  { value: "WAREHOUSE_MANAGER", label: "מנהל מחסן" },
  { value: "LOGISTICS", label: "לוגיסטיקה" },
  { value: "DRIVER", label: "נהג" },
];
...
export default function SettingsPage() {
  ...
  const { profiles, updateProfile, createEmployee, refreshProfiles } = useData();
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div><p className="text-xs text-muted-foreground mb-1">תפקיד</p><p className="text-sm font-medium text-foreground">{roleLabel[managerProfile.role]}</p></div>
            </div>
          </CardContent>
        </Card>
      )}
                  ...
                  <div className="space-y-4 pt-2">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select value={empRole} onValueChange={v => setEmpRole(v as Role)}>
                        ...
                        <SelectContent>{employeeRoleOptions.map(r => <SelectItem key={r.value} value={r.value}>{r.label}</SelectItem>)}</SelectContent>
                      </Select>
                    </div>
                    <div className="space-y-2">
