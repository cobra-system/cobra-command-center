...
import {
  AlertTriangle, Package, Pencil, ExternalLink, ArrowDown,
  Settings, Printer, ShoppingCart, ArrowLeftRight,
} from "lucide-react";
import type { WarehouseZone } from "@/data/warehouseZones";
import type { ZoneInventoryData, ZoneProduct } from "@/hooks/useWarehouseInventory";
import { printZoneLabel } from "./PrintMapUtils";
import ZoneChangeLog from "./ZoneChangeLog";
...
interface ZoneDetailPanelProps {
  ...
  onEdit: () => void;
  onZoneConfig: () => void;
  onReorder?: (lowStockProducts: ZoneProduct[]) => void;
  onTransfer?: () => void;
  canEdit: boolean;
}
...
export default function ZoneDetailPanel({
  ...
  return (
    <Sheet open={open} onOpenChange={(o) => !o && onClose()}>
      <SheetContent side="bottom" className="max-h-[75vh] overflow-hidden flex flex-col">
        <SheetHeader className="flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div className="mr-auto flex gap-1 flex-wrap">
              ...
              {canEdit && !zone.isNonProduct && products.length > 0 && onTransfer && (
          ...
          {/* Notes */}
          {zone.notes && (
            <div className="text-xs text-muted-foreground bg-muted/50 rounded-md px-3 py-2 mt-1 text-right">
              {zone.notes}
            </div>
          )}

          {/* Low stock reorder CTA */}
          {inventoryData?.hasLowStock && onReorder && !zone.isNonProduct && (
            <div className="pt-1">
              <Button
                variant="outline"
                size="sm"
                className="text-destructive border-destructive/40 hover:bg-destructive/5 h-7 text-xs"
                onClick={() => onReorder(lowStockProducts)}
              >
                <ShoppingCart className="w-3 h-3 ml-1" />
                הזמן מחדש ({lowStockProducts.length} פריטים)
              </Button>
            </div>
          )}
        </SheetHeader>

        {/* Product list + change log */}
        <div className="flex-1 overflow-y-auto mt-3 -mx-6 px-6">
          ...
          {/* Change log — always shown, self-loading */}
          <ZoneChangeLog zoneId={zone.id} />
        </div>
      </SheetContent>
    </Sheet>
  );
