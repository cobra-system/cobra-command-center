...
// ── Custom order-marker label rendered inside SVG ────────────────────────────
function OrderLabel({ viewBox, qty, color }: {
  ...
  const { x, y } = viewBox;
  const text = qty > 0 ? `${qty}` : "📦";
  return (
    <g>
      <rect x={x - 14} y={y + 4} width={28} height={16} rx={3}
        fill={color} fillOpacity={0.9} />
      <text x={x} y={y + 15} textAnchor="middle"
        fill="white" fontSize={9} fontWeight="700">{text}</text>
    </g>
  );
}
...
export function ProductConsumptionChart({
  ...
  // Active divisions list (null = all → expand to full array for compare/stacked)
  ...
  // Aggregate monthly data for avg calculations and aggregate mode
  ...
  // Per-division monthly maps (for compare/stacked)
  ...
  // All historical months across active divisions
  const allHistoricalMonths = useMemo(() => {
    const set = new Set<string>();
    for (const r of filteredRows) {
      if (!activeDivisions || activeDivisions.includes(r.division)) {
        set.add(r.month);
      }
    }
    return [...set].sort();
  ...
  // Forecast (only in aggregate mode, only when stockQty provided + enough data)
  const forecastPoints = useMemo(() => {
    if (viewMode !== "aggregate" || stockQty === undefined || monthlyData.length < 2) return [];
    return forecastConsumption(monthlyData, 5);
  }, [monthlyData, stockQty, viewMode]);
  ...
  const chartData = useMemo(() => {
    ...
    // Forecast append (aggregate mode only)
    if (viewMode === "aggregate") {
      for (const fp of forecastPoints) {
        points.push({
          month: fp.month,
          label: formatMonth(fp.month),
          isForecast: true,
          forecastQty: fp.forecastQty,
          forecastLow: fp.forecastLow,
          forecastBand: fp.forecastBand,
          // keep quantity undefined so the history area doesn't extend
        });
      }
    }
  ...
  const orderMarkers = useMemo(() => {
    ...
    for (const o of relatedOrders) {
      ...
      if (existing) {
        existing.qty += qty;
        // prefer more urgent color: orange > blue > green > gray
        const priority = (c: string) => c === "#f97316" ? 3 : c === "#2563eb" ? 2 : c === "#16a34a" ? 1 : 0;
        if (priority(color) > priority(existing.color)) existing.color = color;
      } else {
        byLabel.set(label, { qty, color });
      }
    }
  ...
  return (
    <Card>
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex items-center justify-between flex-wrap gap-x-3 gap-y-1.5">
            ...
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              {/* View mode toggle (only when 2+ divisions) */}
              {showModeToggle && (
      ...
      <CardContent className="pb-4">
          ...
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData as never[]} margin={{ top: 12, right: 16, left: 4, bottom: 4 }}>
              <defs>
                ...
                <linearGradient id="cg-forecast" x1="0" y1="0" x2="0" y2="1">
              ...
              <Tooltip
                content={
                  <ChartTooltip viewMode={viewMode} activeDivisions={activeDivisionsList} />
                }
              />
              ...
              {viewMode === "aggregate" && (
                <>
                  <Area
                    ...
                    connectNulls={false}
                  />
                  {/* Confidence band bottom (transparent) */}
                  ...
                  {/* Confidence band (forecastHigh - forecastLow) */}
                  {forecastPoints.length > 0 && (
                    <Area
                      type="monotone"
                ...
                activeDivisionsList.map((div, i) => {
                  ...
                  return (
                    <Area
                      ...
                      fill={color}
                      fillOpacity={viewMode === "stacked" ? 0.55 : 0.12}
                      dot={false}
                      activeDot={{ r: 4, strokeWidth: 0 }}
                    />
                  );
              ...
              {/* ── Order event markers ──────────────────────────────── */}
              {orderMarkers.map(({ label, qty, color }) => (
                <ReferenceLine
                  key={`order-${label}`}
                  x={label}
                  stroke={color}
                  strokeDasharray="3 3"
                  strokeWidth={1.5}
                  label={<OrderLabel qty={qty} color={color} />}
                />
              ))}
            </AreaChart>
          </ResponsiveContainer>
        ...
        {forecastPoints.length > 0 && viewMode === "aggregate" && (
          <div className="flex items-center gap-3 mt-2 text-[10px] text-muted-foreground">
            ...
            {!stockoutLabel && stockQty !== undefined && (
        ...
        {/* ── Stacked/compare division color legend ────────────────────── */}
