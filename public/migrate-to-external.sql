-- ============================================================
-- COBRA Operations — Full Migration to External Supabase
-- Project: ljpdwezgahrrffnwajho
...
CREATE TABLE IF NOT EXISTS public.products (
  ...
  end_product_image text,
  end_product_url text,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS public.product_issues (
  ...
  severity text NOT NULL DEFAULT 'בינוני',
...
CREATE TABLE IF NOT EXISTS public.center_inventory (
  id uuid NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  center_id uuid NOT NULL REFERENCES public.distribution_centers(id) ON DELETE CASCADE,
  product_id uuid NOT NULL REFERENCES public.products(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS public.inventory_transfers (
  id uuid NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  product_id uuid REFERENCES public.products(id),
  from_center_id uuid REFERENCES public.distribution_centers(id),
  to_center_id uuid REFERENCES public.distribution_centers(id),
  quantity integer NOT NULL DEFAULT 0,
  transferred_by text,
  notes text,
  created_at timestamptz NOT NULL DEFAULT now()
...
CREATE TABLE IF NOT EXISTS public.compliance_product_links (
  id uuid NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
AS $$ SELECT crypt(raw_pin, gen_salt('bf')) $$;

CREATE OR REPLACE FUNCTION public.verify_pin(raw_pin text, hashed_pin text)
RETURNS boolean
LANGUAGE sql IMMUTABLE
SET search_path TO 'public', 'extensions'
AS $$ SELECT hashed_pin = crypt(raw_pin, hashed_pin) $$;
...
DROP TRIGGER IF EXISTS update_compliance_items_updated_at ON public.compliance_items;
CREATE TRIGGER update_compliance_items_updated_at BEFORE UPDATE ON public.compliance_items
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

DROP TRIGGER IF EXISTS update_purchase_documents_updated_at ON public.purchase_documents;
...
DROP TRIGGER IF EXISTS update_supplier_price_quotes_updated_at ON public.supplier_price_quotes;
...
-- NOTE: Auth users must be created first via Supabase Dashboard or the seed-data edge function.
-- The data below assumes the same user IDs from the original project.
-- You need to create these auth users FIRST:
--   admin@cobra.io (password: cobra2026) -> ID: 5eda5d68-188b-47ab-919e-29b7882a343b
--   george@cobra.io (password: pin1111) -> ID: 50138e6e-506b-4b3e-9f15-5acac6804159
--   ziv@cobra.io (password: pin2222) -> ID: 6a9af853-0090-4289-af71-ffc869c7e156
...
INSERT INTO products (id, category, division, name, description, sku, product_type, supplier, supplier_origin, shipping, purchase_price, monthly_sales, monthly_order, stock_qty, incoming_qty, notes, end_product_image, end_product_url, created_at, updated_at) VALUES
  ...
  ('f3147d85-0552-4dbc-9c93-48a503f62f26', 'מיגון ואיתור', 'AWCAS', 'S400', 'מערכת צילום וניטור', 's400', 'מוגמר', 'iStar', 'סין', 'ימי + אווירי', 83, 105, 400, 0, 400, NULL, NULL, NULL, '2026-03-08 17:05:25.933704+00', '2026-03-08 22:31:46.322644+00'),
  ('4e2d8c92-3c5d-4510-a90c-9d2bf5ad8352', 'מיגון ואיתור', 'כפתור', 'KAF32', 'מערכת צילום וניטור', 'ks400', 'מוגמר', 'iStar', 'סין', 'ימי + אווירי', 73, 0, 50, 77, 200, 'SAP לא מיושר מול חברת כפתור', NULL, NULL, '2026-03-08 17:05:26.010353+00', '2026-03-08 22:31:59.263141+00'),
  ...
  ('a7dac02e-b3e7-4870-9976-f13c31f71086', 'מולטימדיה', 'קראסו, דלק מוטורס, לובינסקי', 'מסכי אנדרואיד', 'מערכת מולטימדיה', 'AHM1280A', 'מוגמר', 'AcesVision', 'סין', 'ימי', 130, 42, NULL, 63, 0, NULL, NULL, NULL, '2026-03-08 17:05:26.367424+00', '2026-03-08 22:38:08.639734+00'),
  ...
  ('36e56262-698c-4621-910a-33fbaa099ae5', 'בטיחות', 'דלק מוטורס', 'COBRA Alert', NULL, '4821', 'מוגמר', 'Vodafone', 'איטליה', 'אווירי', 39.5, 530, 775, 63, 0, NULL, NULL, NULL, '2026-03-08 17:05:26.941809+00', '2026-03-08 22:39:47.967318+00'),
  ...
  ('6b97068e-cf65-488a-8a60-3a149a77599c', 'נוחות וקישוריות', 'לובינסקי', 'פותח מצלמה אחורית לברלינגו', NULL, '31217', 'מוגמר', 'BOW', 'סין', 'אווירי', 86, 90, 100, 162, 0, NULL, NULL, NULL, '2026-03-08 17:05:27.098485+00', '2026-03-08 19:29:44.14924+00'),
...
ON CONFLICT (sku) DO UPDATE SET name=EXCLUDED.name, category=EXCLUDED.category, stock_qty=EXCLUDED.stock_qty, incoming_qty=EXCLUDED.incoming_qty;

-- SUPPLIERS (all 53 suppliers)
...
-- PRODUCT_COMPONENTS (all 36 components)
...
INSERT INTO purchase_documents (id, type, quantity, currency, status, file_url, notes, approval_date, approved_by, created_at, updated_at) VALUES
  ('aa265cc2-e5c0-4cff-92e4-e5614615818a', 'PI', 0, 'USD', 'נשלח לספק', NULL, 'קובץ: PI iSV260310A05rev1 spare.pdf', '2026-03-10 15:40:02.124+00', '5eda5d68-188b-47ab-919e-29b7882a343b', '2026-03-10 15:40:01.205682+00', '2026-03-10 15:40:05.571294+00'),
  ('d3c703ee-e37d-42ed-8a92-c3c7f74e5725', 'PI', 0, 'USD', 'ממתין לאישור', NULL, 'קובץ: PI 20260107R01.pdf', NULL, NULL, '2026-03-10 15:41:13.72234+00', '2026-03-10 15:41:13.72234+00'),
  ('09b3bdf3-1a8f-4de2-a2fe-783d5a0c719a', 'PI', 0, 'USD', 'ממתין לאישור', NULL, 'קובץ: 20260310- Proforma Invoice for wire.pdf', '2026-03-11 04:48:38.423+00', '5eda5d68-188b-47ab-919e-29b7882a343b', '2026-03-10 18:18:26.553429+00', '2026-03-11 05:37:31.544642+00')
ON CONFLICT (id) DO NOTHING;
...
-- ============================================================
-- 9. CREATE AUTH USERS (run separately or use seed-data edge function)
-- ============================================================
-- IMPORTANT: Before running data inserts, you need to create these auth users
-- in Supabase Dashboard -> Authentication -> Users:
--
-- 1. admin@cobra.io / cobra2026 (confirm email)
-- 2. george@cobra.io / pin1111 (confirm email)
-- 3. ziv@cobra.io / pin2222 (confirm email)
--
-- The handle_new_user trigger will auto-create profiles.
-- After creating users, update the profile IDs in the inserts above
-- to match the new auth user IDs, OR use the seed-data edge function.
--
-- ALTERNATIVELY: Deploy the seed-data edge function and call it to create
-- users + seed data automatically.
-- ============================================================

-- ============================================================
-- 10. MISSING MIGRATIONS (2026-03-22 to 2026-03-24)
-- ============================================================

-- --- 20260322000000_add_task_depends_on ---
...
-- --- 20260322000000_create_meetings ---
...
-- --- 20260322000000_create_role_permissions_table ---
...
-- --- 20260323100000_add_tracking_number_to_orders ---
...
-- --- 20260323110000_create_task_advancement_log ---
...
-- --- 20260323200000_meetings_participants_documents ---
...
-- --- 20260323_add_document_products_table ---
...
CREATE POLICY "Enable read access for authenticated users" ON public.document_products
  FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY "Enable insert for authenticated users" ON public.document_products
  FOR INSERT WITH CHECK (auth.role() = 'authenticated');
CREATE POLICY "Enable update for authenticated users" ON public.document_products
  FOR UPDATE USING (auth.role() = 'authenticated');
CREATE POLICY "Enable delete for authenticated users" ON public.document_products
  FOR DELETE USING (auth.role() = 'authenticated');

CREATE INDEX idx_document_products_document_id ON public.document_products(document_id);
CREATE INDEX idx_document_products_product_id ON public.document_products(product_id);

-- --- 20260324000000_create_goals_table ---
