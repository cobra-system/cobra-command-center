...
### Fixed
- שמות הקבצים של DHL זוהו שגוי בשלושה מתוך ארבעה, ומספר התיק לא זוהה כלל. DHL מקבץ לפי שטר מטען אווירי בן 10 ספרות בעוד הזיהוי קיבל 4-8 בלבד; בנוסף `_ppwk` הוא דווקא החשבונית המסחרית של הספק ו-`_proforma` הוא חשבון החיובים של DHL — שני שמות שקוראים הפוך ממה שהם מכילים. שניהם מקובעים עכשיו לפי שם, וכך גם `_sad` (רשימון).
- סוג המסמך ניתן לעריכה בשורה עצמה, כי כל סיווג הוא ניחוש משם הקובץ.
- סוג המשלוח (ימי/אווירי) ניתן להחלפה בקליק מתוך התיק. הוא במכוון אינו מנוחש משמות הקבצים: `HAWB_460509.pdf` של Total Care הוא שטר מטען ימי בעוד `_awb` של DHL באמת אווירי, וסוג המשלוח קובע אם התעריף מחושב לק"ג או ל-CBM.
- מסמכי יבוא נדחו על ידי ה-CHECK של `purchase_documents.document_subtype`, שלא הכיר את הסוגים החדשים. מכל אצווה נקלטו רק שטר המטען ורשימת האריזה — שני הסוגים שכבר היו ברשימה המותרת — והשאר נכשלו. ה-constraint הורחב, ויש טסט שנועל את שתי הרשימות יחד.
- מסמך שכבר קיים בתיק לא נוסף שוב בהעלאה חוזרת, כך שניסיון חוזר אחרי כישלון לא מייצר כפילויות.
- תיק שנוצר לאצווה שכולה נכשלה נמחק במקום להישאר כקליפה ריקה על ההזמנה.
- העלאת מסמכים בשמות עבריים אבדה בשקט. `sanitizeFileName` מוריד כל תו לא-אנגלי, כך שכל שם בעברית הצטמצם ל-`_.pdf`; בצירוף חותמת זמן במילישניות, אצווה של קבצים עבריים קיבלה נתיב אחסון זהה ו-Supabase דחה את כל מי שאחרי הראשון. גרירה של 10 קבצים קלטה 2. מפתח האחסון מבוסס עכשיו על מזהה אקראי ואינו יכול להתנגש.
- אצווה שנגררה בזמן שהעלאה קודמת רצה נזרקה בשקט. עכשיו היא נכנסת לתור.
- כשלי העלאה מוצגים ברשימה קבועה במקטע עד ההעלאה התקינה הבאה, במקום טוסט חולף.

### Added
- **מודול תיקי יבוא** — קליטת מסמכי היבוא שמגיעים מעמיל המכס אחרי שמשלוח נוחת, וקיטלוגם יחד עם ההזמנה. בכל הזמנה נוסף מקטע "מסמכי יבוא": פתיחת תיק (לפי מספר התיק של המשלח), העלאת המסמכים — חשבונית מסחרית, רשימת אריזה, שטר מטען, רשימון יבוא וחשבוניות ההובלה, המסוף והמשלח — והזנת שורות העלות. השיוך להזמנה ידני בשלב זה, ומתועד כדי לשמש בסיס להתאמה אוטומטית בהמשך.
- שלוש טבלאות חדשות: `import_files`, `import_file_orders` (רבים-לרבים — מכולה אחת נושאת כמה הזמנות) ו-`import_cost_lines`, בתוספת `import_file_id` ל-`purchase_documents` כך שקבצי היבוא נשמרים במודול המסמכים הקיים.
- חישוב עלות נחיתה שמנטרל שתי מלכודות: מע"מ יבוא מסומן כמתקזז ולא מנפח את עלות המוצר, וחיוב שכבר מופיע בחשבונית המשלח המרכזת מסומן ככלול ואינו נספר פעמיים.
- טאב נפרד "מסמכי יבוא" במודול המסמכים, מקובץ לפי תיק. מסמכי יבוא הוצאו מהרשימה הכללית כדי ש-7-8 קבצים לכל משלוח לא יטביעו את ה-PI וה-PO.
- **עלות ההובלה כמספר עצמאי** — מופרדת ממכס, אגרות ומע"מ, ומוצגת בכרטיס "פרטי הזמנה" לצד סה"כ ושיטת המשלוח, בכותרת המקטע בדף ההזמנה, בכותרת כל תיק ובטאב מסמכי היבוא, בלי צורך לפתוח כלום. לצידה סוג המשלוח (ימי/אווירי) ותעריף ליחידה — לק"ג באווירי, ל-CBM בימי — כי סכום בלבד לא מאפשר להשוות בין משלוחים.
- כלי MCP `get_shipping_cost_report` לניתוח עלויות הובלה לאורך זמן: סיכום לפי סוג משלוח, ממוצע משוקלל לק"ג ול-CBM, ואחוז מערך הטובין.
- מודול MCP חדש `import-files` עם 13 כלים, ובהם `get_import_file_costs` לעלות נחיתה ו-`find_orders_for_import_file` שמדרג הזמנות מועמדות לפי מזהי משלוח משותפים.
