...
### Added
- **תהליך אישור הרשמה** — דף הכניסה החדש כולל כניסה עם Google, אימייל/סיסמה, "שכחת סיסמה?" וקישור להרשמה. הרשמות חדשות אינן נכנסות אוטומטית למערכת — נוצרת רשומה ב-`signup_requests` ומייל אישור נשלח ל-`noamshemla@gmail.com` ול-`noam@cobra.co.il`. מנהלי המערכת מקבלים טאב "בקשות הרשמה" בהגדרות, שם ניתן לאשר (עם בחירת תפקיד, אגף וסיסמה ראשונית שתישלח למבקש) או לדחות את הבקשה. כניסה דרך Google של משתמש שאין לו פרופיל יוצרת אוטומטית בקשת הרשמה ומנתקת אותו עד לאישור.
