...
### Fixed
- כלי ה-MCP דיווחו על מסד נתונים ריק במקום על תקלה. שאילתות שנשלחו במקביל נפתחו עם `res.data || []`, כך שכשל רשת או הרשאה נבלע וה-`error` נזרק לפח: `get_dashboard_kpis` החזיר 0 הזמנות ו-0 מוצרים, ו-`list_alerts` החזיר "אין התראות" — תשובות שלא ניתן להבחין בינן לבין מצב תקין. 114 מקומות ב-19 מודולים עברו ל-`unwrapRows()`, שזורק שגיאה עם שם השאילתה והסיבה.
